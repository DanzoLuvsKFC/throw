...
// Interactive 3D dome of images (vignette removed)
export default function DomeGallery({
  ...
  function buildItems(pool, seg) {
    ...
    const normalizedImages = pool.map((image, i) =>
      typeof image === "string" ? { src: image, alt: `image ${i + 1}` } : { src: image?.src || "", alt: image?.alt || "" }
    );
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      root.style.setProperty("--tile-gap", `${tileGapPx}px`);
  ...
  const startInertia = useCallback(
    (vx, vy) => {
      ...
      let frames = 0;
      const d = Math.min(Math.max(dragDampening ?? 0.6, 0), 1);
      ...
      const step = () => {
        ...
        const nextX = Math.min(
          Math.max(rotationRef.current.x - vY / 200, -maxVerticalRotationDeg),
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        const nextX = Math.min(
          Math.max(startRotRef.current.x - dyTotal / dragSensitivity, -maxVerticalRotationDeg),
          maxVerticalRotationDeg
        );
  ...
  const cssStyles = `
    ...
    .item__image { position: absolute; inset: var(--tile-gap, 10px); border-radius: var(--tile-radius, 12px); overflow: hidden; cursor: pointer; backface-visibility: hidden; -webkit-backface-visibility: hidden; transition: transform 300ms; pointer-events: auto; -webkit-transform: translateZ(0); transform: translateZ(0); }
    .item__image--reference { position: absolute; inset: var(--tile-gap, 10px); pointer-events: none; }
  `;
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Optional vignette that blends with section background */}
          {vignette && (
            <>
              <div
                className="pointer-events-none absolute inset-0 m-auto z-[3]"
                style={{
                  backgroundImage: `radial-gradient(rgba(235,235,235,0) 62%, ${vignetteColor} 100%)`,
                }}
              />
              <div
                className="pointer-events-none absolute left-0 right-0 top-0 h-[120px] z-[5] rotate-180"
                style={{ background: `linear-gradient(to bottom, transparent, ${vignetteColor})` }}
              />
              <div
                className="pointer-events-none absolute left-0 right-0 bottom-0 h-[120px] z-[5]"
                style={{ background: `linear-gradient(to bottom, transparent, ${vignetteColor})` }}
              />
            </>
          )}
